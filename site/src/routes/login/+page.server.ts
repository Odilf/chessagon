...
function getAsString(formData: FormData, key: string) {
	...
	throw error(400, `formData with key ${key} was ${type} instead of string (specifically, ${value})`);
}
...
export const actions = {
	register: async ({ request, locals: { supabase } }) => {
		...
		const { data, error: supabaseErr } = await supabase.auth.signUp({ email, password })
	...
	login: async ({ request, locals: { supabase } }) => {
		...
		const { data, error: supabaseErr } = await supabase.auth.signInWithPassword({ email, password })

		if (supabaseErr) {
			return fail(400, { email, error: { message: supabaseErr.message } })
		}

		console.log('logged in', await supabase.auth.getSession());
