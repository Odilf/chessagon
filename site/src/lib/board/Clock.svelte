...
<div class="card variant-ghost-surface p-4 flex lg:flex-col lg:px-8">
  <div
    class="{turn === playerColor &&
      'opacity-50'} transition flex-1 font-bold text-2xl lg:text-center -z-10"
  >
    Opponent
  ...
  <div class="flex flex-col lg:flex-row mx-auto items-center">
    <div
      class="{turn === playerColor
        ? 'lg:rotate-90'
        : 'lg:-rotate-90 rotate-180'} w-fit h-fit transition-transform py-4 mx-4 text-surface-400 -z-10"
    >
      <svg
    ...
    <div class="text-xl text-surface-400">{Math.ceil(moves.length / 2)}</div>
  </div>

  <div
    class="{turn !== playerColor &&
      'opacity-50'} transition flex-1 font-bold text-2xl text-right lg:text-center -z-10"
  >
    You
    <div class="font-mono font-thin text-3xl">
